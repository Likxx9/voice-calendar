...
5. **联网检索智能体 (M9 Search Agent)**：
   * 利用 Tavily Search Adapter 或 Google Search API，结合大语言模型非结构化文本的语义理解，实现时效性活动信息的动态获取与日程提取，基于 Hexagonal Ports 架构隔离，技术成熟且边界清晰。
...
* **联网检索消除时效盲区**：打破日历工具仅可创建本地静态日程的瓶颈，智能嗅探用户指令的时效性特征，实时检索互联网活动（如漫展、音乐节、体育赛事），并实现结构化日程实体的一键完美添加。
...
* **开发顺序**：
  ...
  5. **【联网检索智能体】** [已于前端沙盒完成] 集成外部 Tavily / Google 网页搜索引擎适配器，结合 LLM 对非结构化搜索网页进行深度日程实体提取，并在遇到歧义时调用状态机多轮澄清追问，支持一键将网页中的实践活动结构化写入日历。
