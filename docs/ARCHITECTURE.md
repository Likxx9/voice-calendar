...
| Agent LLM | Anthropic Claude API | Haiku(快)/Sonnet(强) 双模型路由 |
| 联网搜索 | Tavily/SerpAPI/Bing | AI原生搜索，结果缓存6h |
...
┌─────────────────────────────────────────────────────────────────────────┐
│                       L3: Agent 编排层（新增）                            │
│  职责：LLM任务规划、DAG任务图构建、并行调度器、联网搜索                     │
│  输入：NLU 意图列表 + 对话历史                                           │
│  输出：任务组 + 工具执行结果                                              │
│  核心：agent_service.py / tool_registry.py / search_service.py           │
└─────────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
